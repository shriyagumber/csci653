...
# Problem Description
Non-adiabatic molecular dynamics requires accurate time-dependent electronic Hamiltonians, but ab initio quantum chemical calculations are often too expensive to compute at every nuclear geometry. The problem is to learn the electronic Hamiltonian, energies, couplings, and derivative terms using machine learning models trained on high quality quantum data. Once learned, this ML Hamiltonian enables fast, stable, and accurate non-adiabatic dynamics across long trajectories and high-dimensional systems. Application on C-defect modified 2D- graphitic Carbon Nitride. 
...
Dataset Information: ~30,000 data points collected from ab initio moleclar dynamics (AIMD) within the framework of density functional theory (DFT), at varied temperature: from 100 K to 1200 K. 
The DFT calculations are done using the GGA-PBE approximated DFT functional as implemented in the VASP software. 
...
Following the DeepH formalism, the machine-learning Hamiltonian leverages the locality of the DFT Hamiltonian by learning only short-range Hamiltonian matrix elements expressed in a localized orbital basis. Because each Hamiltonian block depends solely on the atomic neighborhood within a finite cutoff, training can be performed on relatively small systems while maintaining transferability to much larger structures. This locality-preserving representation allows the ML Hamiltonian to scale linearly with system size and enables accurate Hamiltonian construction for systems containing hundreds to thousands of atoms. https://www.nature.com/articles/s43588-022-00265-6
